...
const SearchResults = () => {
  ...
  const isFlightSearch = (filters.property_type || '').toLowerCase() === 'flight';

  return (
    <div className={`min-h-screen ${isFlightSearch ? 'bg-[#F4F6F9]' : 'bg-white mobile-footer-spacing'}`}>
      {/* Sticky header appears after scroll (desktop only) */}
      {showStickySearchHeader && !isFlightSearch && (
