...
const Allowances = () => {
    ...
    const { data: allowances, isLoading } = useQuery('hms-allowances', () => 
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.post('/hms/hr/allowances', formData);
            showSuccess('Allowance created');
            setShowForm(false);
            queryClient.invalidateQueries('hms-allowances');
        } catch (error) {
            showError('Failed to create allowance');
        }
    };
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800">Allowances</h1>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {isLoading ? (
                            ...
                            <tr key={a.id}>
                                ...
                                <td className="px-6 py-4 capitalize">{a.amount_type}</td>
            ...
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-md">
                        <h2 className="text-xl font-bold mb-4">Add Allowance</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <button type="submit" className="w-full btn-primary py-3 rounded-xl font-bold">Save</button>
                        </form>
                    </div>
                </div>
            )}
