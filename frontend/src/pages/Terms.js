...
import { FiCheckCircle, FiFileText, FiShield, FiTrendingUp, FiInfo, FiCreditCard, FiClock, FiUsers, FiHome, FiAlertTriangle, FiDollarSign, FiLock, FiGlobe, FiPhone } from 'react-icons/fi';
...
const Terms = () => {
    ...
    const getIconForSection = (title) => {
        ...
        if (lowerTitle.includes('about')) return <FiInfo />;
        if (lowerTitle.includes('book') || lowerTitle.includes('pay')) return <FiCreditCard />;
        if (lowerTitle.includes('check')) return <FiClock />;
        if (lowerTitle.includes('guest') || lowerTitle.includes('user')) return <FiUsers />;
        if (lowerTitle.includes('property') || lowerTitle.includes('house')) return <FiHome />;
        if (lowerTitle.includes('damage') || lowerTitle.includes('liab')) return <FiAlertTriangle />;
        if (lowerTitle.includes('cancel') || lowerTitle.includes('refund')) return <FiDollarSign />;
        if (lowerTitle.includes('security') || lowerTitle.includes('priva')) return <FiShield />;
        if (lowerTitle.includes('law') || lowerTitle.includes('third')) return <FiGlobe />;
        if (lowerTitle.includes('contact')) return <FiPhone />;
        return <FiFileText />;
    };

    useEffect(() => {
        if (settings?.terms_of_service) {
            const lines = settings.terms_of_service.split('\n');
            const sections = [];
            ...
            setParsedData({
                sections: [
                    { id: 'acceptance', title: '1. Acceptance', icon: <FiCheckCircle />, lines: ['By accessing...', 'If you do not agree...'] }
                ],
        ...
        <div className="bg-gray-50/50 min-h-screen font-sans selection:bg-blue-200 pb-20">
            ...
            <div className="bg-gradient-to-br from-gray-900 to-navy-900 relative py-8 overflow-hidden rounded-b-[3rem] shadow-2xl mb-6">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center animate-[fadeInUp_1s_ease-out_forwards]">
                    <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-2 text-white">Terms & Conditions</h1>
                    <p className="text-blue-100 font-light max-w-2xl mx-auto text-sm opacity-90">
                        {siteName} platform policies and guidelines for guests and hosts.
                    </p>
                    ...
                    {parsedData.sections.length > 1 && (
                        <div className="lg:w-1/4 hidden lg:block">
                            <div className="sticky top-28 bg-white p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 animate-[fadeInUp_1s_ease-out_0.2s_forwards] opacity-0">
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-6 ml-3">Table of Contents</h3>
                    ...
                    <div className={parsedData.sections.length > 1 ? "lg:w-3/4" : "w-full max-w-4xl mx-auto"}>
                        ...
                        <div className="mt-12 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-3xl p-10 flex flex-col md:flex-row items-center justify-between shadow-xl shadow-blue-500/20 animate-[fadeInUp_1s_ease-out_1s_forwards] opacity-0">
                            <div className="mb-6 md:mb-0">
                                ...
                                <p className="text-blue-100 opacity-90">Our support team is happy to clarify any points of confusion.</p>
                            </div>
